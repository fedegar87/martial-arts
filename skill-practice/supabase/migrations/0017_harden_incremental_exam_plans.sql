-- Keep exam plans strictly incremental by grade.
-- This also cleans any saved exam plan rows produced while requirements were
-- cumulative or otherwise polluted.
...
CREATE INDEX IF NOT EXISTS idx_user_plan_items_user_source_visible
  ON user_plan_items(user_id, source)
...
DELETE FROM user_plan_items item
WHERE item.source = 'exam_program'
  AND NOT EXISTS (
    SELECT 1
    FROM expected_exam_items expected
    WHERE expected.user_id = item.user_id
